...
public class PanelRelatorioPedido extends JPanel {
    ...
    public PanelRelatorioPedido(JLayeredPane layeredPane) {
	...
	public void geraDocumento(){
		String html = "<!DOCTYPE html><html lang='pt-br'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>Document</title></head><body><style>  *, *:before, *:after {margin: 0;}#main {width: 100%;position:absolute;left: 140px;}table {width: 100%;}th {background-color: gray;}.cn {background-color: aqua;}table, th, td {text-align: center;border: 1px solid black;border-collapse: collapse;}h1 {text-align: center;}p {margin-top: 5px;margin-bottom: 5px;}#sidebar {position: absolute;left: 0;top: 0;width: 140px;background-color: lightblue;border-right: 1px solid black;}</style><div id=\"sidebar\"><h4>Filtros utilizados</h4><strong>data:</strong>"+"<p>min:"+tf_datamin.getText()+"</p><p>max:"+tf_datamax.getText()+"</p><strong>Quantidade de itens</strong><p>min:"+tf_quantMin.getText()+"</p><p>max:"+tf_quantMax.getText()+"</p><strong>Itens distintos</strong><p>min:"+tf_proddismin.getText()+"</p><p>max:"+tf_proddismax.getText()+"</p><strong>Subtotal</strong><p>min:"+tf_subtotalmin.getText()+"</p><p>max:"+tf_subtotalmin.getText()+"</p><strong>CPF:</strong>";
		...
		try {
			...
			for (ConsultaPedido oop :pedidosConsultados) {
				if (corsin) {
					html+= "<tr>";
					html+=" <td>"+oop.getId()+"</td><td>"+oop.getCpf()+"</td><td>"+oop.getData()+"</td><td>"+oop.getSubTotalf()+"</td><td>"+oop.getQuanidadeDeItens()+"</td><td>"+oop.getQuanidadeDeItensdist()+"</td><td>"+oop.getConcluido()+"</td></tr>";
